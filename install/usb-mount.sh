...
do_mount()
{
    ...
    if [[ -z "${LABEL}" ]]; then
        LABEL=${DEVBASE}
    elif /bin/grep -q " /home/shares/Public/${LABEL} " /etc/mtab; then
        # Already in use, make a unique one
        LABEL+="-${DEVBASE}"
    fi
    MOUNT_POINT="/home/shares/Public/${LABEL}"
...
do_unmount()
{
    ...
    # Delete all empty dirs in /home/shares/Public that aren't being used as mount points. 
    for f in /home/shares/Public/* ; do
